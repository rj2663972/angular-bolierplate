...
import { CacheStorageAbstract } from './storage/cache-storage-abstract';
import { CacheSessionStorage } from './storage/cache-session-storage';
import { CacheLocalStorage } from './storage/cache-local-storage';
import { CacheMemoryStorage } from './storage/cache-memory-storage';
import { StorageValueInterface } from './interfaces/storage-value.interface';

const CACHE_PREFIX = 'AVMCache_';

@Injectable()

export class CoreCacheService {

    // 4.5 hours in secons, because back it's 5 hours timetolife
    ...
    constructor(
        @Optional() private _storage: CacheStorageAbstract
    ) {
        this._initValidStorage();
    }
    ...
    /**
     * Init a valid storage cache, three options Session storage, Local Storage or Memory.
     * @private
     */
    private _initValidStorage() {
        if (!this._storage) {
        ...
        if (!this._storage.isEnabled()) {
            this._storage = new CacheMemoryStorage();
        }
    }
